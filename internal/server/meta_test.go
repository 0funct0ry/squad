...
func TestMetaHandler_NormalAndEmpty(t *testing.T) {
	...
	srv := NewServer(database, dbPath, false, false)
...
func TestMetaHandler_JournalModes(t *testing.T) {
	...
	srvDel := NewServer(dbDel, dbPathDel, false, false)
	...
	srvWal := NewServer(dbWal, dbPathWal, false, false)
...
func TestMetaHandler_SizeBytesUpdates(t *testing.T) {
	...
	srv := NewServer(database, dbPath, true, false) // Write mode
	ts := httptest.NewServer(srv.Handler())
	defer ts.Close()
...
func TestMetaHandler_PathAbsolute(t *testing.T) {
	...
	srv := NewServer(database, absPath, false, false)
	ts := httptest.NewServer(srv.Handler())
	defer ts.Close()
