...
func TestMetaHandler_JournalModes(t *testing.T) {
	...
	srvDel := NewServer(dbDel, dbPathDel, false)
	...
	srvWal := NewServer(dbWal, dbPathWal, false)
...
func TestMetaHandler_SizeBytesUpdates(t *testing.T) {
	...
	srv := NewServer(database, dbPath, true) // Write mode
	ts := httptest.NewServer(srv.Handler())
	defer ts.Close()
...
func TestMetaHandler_PathAbsolute(t *testing.T) {
	...
	srv := NewServer(database, absPath, false)
	ts := httptest.NewServer(srv.Handler())
	defer ts.Close()
