...
func (s *Server) setupSandboxRoutes(api *gin.RouterGroup) {
	...
	scoped.Use(s.sandboxResolveMiddleware())
	{
		...
		scoped.GET("/export/all", scopedHandler((*Server).handleExportAll))
		scoped.POST("/export/query", scopedHandler((*Server).handleQueryExport))
		scoped.GET("/functions", scopedHandler((*Server).handleFunctionsCatalog))
		scoped.POST("/functions/try", scopedHandler((*Server).handleFunctionsTry))
		...
		scoped.POST("/tables/:name/duplicate", scopedHandler((*Server).handleDuplicateTable))
